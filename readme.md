...
企业微信仅发送纯文字,每件商品按下面的格式展示,商品之间空一行,链接直接显示完整URL:
...
赞评：42/12
平台：京东
链接：https://www.smzdm.com/p/123456/
```

本次全部优惠仅在文字超过2048个UTF-8字节时按完整商品条目拆分,不按商品数量分组,消息间隔至少3.1秒发送。该渠道同样遵循过滤规则和`MIN_PUSH_SIZE`阈值,本次所有文字消息均发送成功后才标记为已推送。邮箱和WxPusher仍使用HTML表格,保留每100条分批推送。接口要求见[企业微信群机器人配置说明](https://developer.work.weixin.qq.com/document/path/91770)。

抓取优惠信息仍需安装Chrome及匹配的ChromeDriver,GitHub Actions会自动安装。企业微信文字推送不需要浏览器截图或中文字体。可使用`mvn verify`运行测试并打包。
