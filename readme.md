...
企业微信会先将优惠信息渲染成四列表格图片（标题、价格、赞/评、平台）,每张图最多8个商品,随后单独发送一条商品详情链接消息,链接序号与图片对应。表格图直接上传到企业微信,不需要公网图片存储;超过2MB时自动拆成更小的商品组重新渲染。图片与链接消息均间隔至少3.1秒发送。该渠道同样遵循过滤规则和`MIN_PUSH_SIZE`阈值,本批图片和链接均发送成功后才标记为已推送。接口要求见[企业微信群机器人配置说明](https://developer.work.weixin.qq.com/document/path/91770)。
